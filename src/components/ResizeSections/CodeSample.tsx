...
export const CodeSample = ({ project, page, component, nestedComponentFile }: CodeSampleProps) => {
  let code, code_path;
  // Single file code
  if (page && component && component.page_id !== 'theme') {
    code_path = require(`!!raw-loader!pages/preview/projects/${project.id}/pages/${page.id}/${component.filename}`);
  }
  // Multiple files code
  if (component && nestedComponentFile) {
    if (component.page_id === 'theme') {
      code_path = require(`!!raw-loader!pages/preview/projects/${project.id}/${component?.filename}/${nestedComponentFile}`);
    } else if (component.page_id !== 'theme') {
      code_path = require(`!!raw-loader!pages/preview/projects/${project.id}/pages/${page?.id}/${component?.filename}/${nestedComponentFile}`);
    }
  }
